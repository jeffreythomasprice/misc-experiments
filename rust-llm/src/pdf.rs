...
pub async fn get_page_count(path: &Path) -> Result<u32> {
    ...
    let output = exec(
        ...
        &[
            path.to_str()
                .ok_or(anyhow!("failed to get str from path: {:?}", path))?,
            "dump_data",
        ],
    )
    .await?;
...
pub async fn extract_pdf_pages_into_new_pdf(
    input_path: &Path,
    output_dir: &Path,
    first_page: u32,
    last_page: u32,
    ...
    let input_file_name = input_path
        .file_name()
        .ok_or(anyhow!(
            "failed to get file name for input path: {:?}",
            input_path
        ))?
        .to_string_lossy()
        .to_string();
    let input_file_extension = input_path
        .extension()
        .ok_or(anyhow!(
            "failed to get file extension for input path: {:?}",
            input_path
        ))?
        .to_string_lossy()
        .to_string();
    ...
    let file_name_without_extension = input_file_name.chars().collect::<Vec<_>>()
        [0..(file_name_chars.len() - file_ext_chars.len() - 1)]
        .iter()
        .collect::<String>();
    let output_file_name =
        format!("{file_name_without_extension}-{first_page}-{last_page}.{input_file_extension}");
    ...
    exec(
        "pdftk",
        &[
            input_path.to_str().ok_or(anyhow!(
                "failed to get str from input path: {:?}",
                input_path
            ))?,
            "cat",
...
pub async fn extract_pdf_text(path: &Path) -> Result<String> {
    exec(
        ...
        &[
            path.to_str()
                .ok_or(anyhow!("failed to get str from input path: {:?}", path))?,
            "-",
        ],
    )
    .await
