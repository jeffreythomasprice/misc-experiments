...
def many0[T](p: Parser[T]) =
	repeat(p, Range(RangeOptions.Unbounded, RangeOptions.Unbounded))

def many1[T](p: Parser[T]) =
	repeat(p, Range(RangeOptions.Bounded(1), RangeOptions.Unbounded))
