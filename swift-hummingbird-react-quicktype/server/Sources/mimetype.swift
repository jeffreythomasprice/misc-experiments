...
private func parseHeadersIntoMediaTypes(headers: [String], context: any RequestContext)
    -> [MediaType]
{
...
extension HTTPFields {
    func contentType(context: any RequestContext) throws -> MediaType? {
        let results = parseHeadersIntoMediaTypes(
            headers: self[values: .contentType], context: context)
        if results.count >= 2 {
...
struct MIMETypeRequestDecoder: RequestDecoder {
    ...
    where T: Decodable {
        ...
        case .applicationJson:
            return try await newJSONDecoder().decode(type, from: request, context: context)
