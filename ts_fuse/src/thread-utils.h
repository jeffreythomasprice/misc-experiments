...
template <class T>
T await(const Napi::ThreadSafeFunction& f,
	...
	f.BlockingCall(
		(void*)nullptr,
		[invokeThreadSafeFunctionCallback, castResultToNonJSValueCallback,
		 &result, &s](const Napi::Env& env, Napi::Function f, void*) {
			promiseThen(env, invokeThreadSafeFunctionCallback(env, f),
						[castResultToNonJSValueCallback, &result,
						 &s](const Napi::Value& r) {
